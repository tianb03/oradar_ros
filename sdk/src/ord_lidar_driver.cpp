#include <math.h>
#include "ord_lidar_driver.h"
#include <signal.h>
#include <sys/prctl.h>
#include <sys/epoll.h>
#include <sys/time.h>

namespace ord_sdk
{
#define TMPBUFF_SIZE (10240)
	static const uint8_t CrcTable[256] =
		{
			};

	uint8_t xor_check(uint8_t *data, uint16_t len)
	{
		uint8_t check = 0;
		for(int i = 0; i< len; i++)
		{
			check ^= data[i];
		}
		return check;
	}

	OrdlidarDriver::OrdlidarDriver()
	{
		serial_port_ = NULL;
		rx_thread_ = NULL;
		baudrate_ = 0;
		serial_fd_ = -1;
		is_connected_ = false;
		valid_data_ = 1;
		memset(&parsed_data_, 0, sizeof(parsed_data_st));
		memset(&temp_data, 0, sizeof(full_scan_data_st));
		memset(&full_scan_data_, 0, sizeof(full_scan_data_st));
		memset(&one_scan_data_, 0, sizeof(one_scan_data_st));
		int ret = pthread_condattr_init(&cond_cattr_);
		if (ret != 0)
		{
			printf("Failed to init condattr...\n");
		}
		pthread_condattr_setclock(&cond_cattr_, CLOCK_MONOTONIC);
		pthread_mutex_init(&one_scan_mtx_, NULL);
		pthread_mutex_init(&full_scan_mtx_, NULL);
		pthread_cond_init(&one_scan_cond_, &cond_cattr_);
		pthread_cond_init(&full_scan_cond_, &cond_cattr_);
	}

	OrdlidarDriver::OrdlidarDriver(const std::string &port_name, const int &baudrate)
	{
		serial_port_ = NULL;
		rx_thread_ = NULL;
		baudrate_ = 0;
		serial_fd_ = -1;
		is_connected_ = false;
		memset(&parsed_data_, 0, sizeof(parsed_data_st));
		memset(&temp_data, 0, sizeof(full_scan_data_st));
		memset(&full_scan_data_, 0, sizeof(full_scan_data_st));
		memset(&one_scan_data_, 0, sizeof(one_scan_data_st));

		SetSerialPort(port_name, baudrate);
	}

	OrdlidarDriver::~OrdlidarDriver()
	{
		Disconnect();
		if (serial_port_)
		{
			delete serial_port_;
			serial_port_ = NULL;
		}
	}

	bool OrdlidarDriver::SetSerialPort(const std::string &port_name, const int &baudrate)
	{
		port_name_ = port_name;
		baudrate_ = baudrate;

		return true;
	}

	bool OrdlidarDriver::uart_data_handle(unsigned char *data, int len)
	{
		// printf("uart_data_handle, len= %d\n", len);
		for (int i = 0; i < len; i++)
		{
			bin_buf_.push_back(*(data + i));
		}

		if (bin_buf_.size() < sizeof(OradarLidarFrame))
		{
			return false;
		}

		if (bin_buf_.size() > sizeof(OradarLidarFrame) * 100)
		{
			bin_buf_.clear();
			return false;
		}

		uint32_t pos = 0;
		int flag = 0;
		uint32_t frame_size = 0, temp_size = 0;
		unsigned char per_block_buff[MAX_BLOCK_SIZE] = {0};
		int n = 0;
		unsigned char crc_check = 0;
		static float last_angle = 0;

		while (pos < (bin_buf_.size() - 2))
		{
			pos = 0;
			while (pos < (bin_buf_.size() - 2))
			{
				if (parsed_data_.head_flag == HEAD_FLAG_NONE)
				{
					if ((bin_buf_[pos] == 0x54) && (bin_buf_[pos + 1] == 0x2C))
					{
						parsed_data_.head_flag = HEAD_FLAG_OK;
						parsed_data_.version = VERSION_MS200;
						break;
					}
				}
				else
				{
					break;
				}
				pos++;
			}

			if (pos != 0)
			{
				for (int i = 0; i < pos; i++)
				{
					bin_buf_.erase(bin_buf_.begin());
				}
			}

			if (bin_buf_.size() < sizeof(OradarLidarFrame))
			{
				return false;
			}
			if (VERSION_MS200 == parsed_data_.version)
			{
				OradarLidarFrame *pkg = (OradarLidarFrame *)bin_buf_.data();
				crc_check = 0;
				for (uint32_t i = 0; i < sizeof(OradarLidarFrame) - 1; i++)
				{
					crc_check = CrcTable[(crc_check ^ bin_buf_[i]) & 0xff];
				}

				if (crc_check == pkg->crc8)
				{
					parsed_data_.point_num = POINT_PER_PACK;
					parsed_data_.speed = (double)pkg->speed / 360.0; // Hz
					parsed_data_.start_angle = pkg->start_angle / 100.0;
					parsed_data_.end_angle = pkg->end_angle / 100.0;
					parsed_data_.timestamp = pkg->timestamp;

					if (temp_data.vailtidy_point_num + parsed_data_.point_num < POINT_CIRCLE_MAX_SIZE)
					{
						point_data_parse_frame_ms200(&temp_data.data[temp_data.vailtidy_point_num], (unsigned char *)pkg->point, parsed_data_.point_num * 3, parsed_data_.start_angle, parsed_data_.end_angle);
						if (parsed_data_.point_num < POINT_PKG_MAX_SIZE)
						{
							// one package data
							memcpy(&one_scan_data_.data[0], &temp_data.data[temp_data.vailtidy_point_num], sizeof(point_data_t) * parsed_data_.point_num);
							one_scan_data_.vailtidy_point_num = parsed_data_.point_num;
							one_scan_data_.speed = parsed_data_.speed;
							one_scan_ready_ = true;
							pthread_mutex_lock(&one_scan_mtx_);
							pthread_cond_signal(&one_scan_cond_);
							pthread_mutex_unlock(&one_scan_mtx_);
						}

						for (int i = 0; i < POINT_PER_PACK; i++)
						{
							if ((last_angle - temp_data.data[temp_data.vailtidy_point_num + i].angle) > 1)
							{
								// printf("\n#################### MS200: %d #################\n", full_scan_data_.vailtidy_point_num);
								full_scan_ready_ = false;
								memcpy(&full_scan_data_.data, &temp_data.data, sizeof(point_data_t) * (temp_data.vailtidy_point_num + i));
								full_scan_data_.speed = parsed_data_.speed;
								full_scan_data_.vailtidy_point_num = temp_data.vailtidy_point_num + i;
								if (full_scan_data_.data[0].angle > 5)
								{
									// printf("not a full scan, start angle:%f, end angle:%f\n", full_scan_data_.data[0].angle,
									//	   full_scan_data_.data[full_scan_data_.vailtidy_point_num - 1].angle);
								}
								else
								{
									full_scan_ready_ = true;
									pthread_mutex_lock(&full_scan_mtx_);
									pthread_cond_signal(&full_scan_cond_);
									pthread_mutex_unlock(&full_scan_mtx_);
								}

								last_angle = temp_data.data[temp_data.vailtidy_point_num + POINT_PER_PACK - 1].angle;
								memcpy(&temp_data.data[0], &temp_data.data[temp_data.vailtidy_point_num + i], sizeof(point_data_t) * (POINT_PER_PACK - i));
								temp_data.vailtidy_point_num = POINT_PER_PACK - i;
								flag = 1;
								break;
							}
							last_angle = temp_data.data[temp_data.vailtidy_point_num + i].angle;
						}

						if (flag)
						{
							flag = 0;
						}
						else
						{
							temp_data.vailtidy_point_num += parsed_data_.point_num;
						}
					}
					else
					{
						temp_data.vailtidy_point_num = 0;
					}

					parsed_data_.head_flag = HEAD_FLAG_NONE;
					parsed_data_.version = VERSION_NONE;
					for (uint32_t i = 0; i < sizeof(OradarLidarFrame); i++)
					{
						bin_buf_.erase(bin_buf_.begin());
					}

					if (bin_buf_.size() < sizeof(OradarLidarFrame))
					{
						break;
					}
				}
				else
				{
					parsed_data_.head_flag = HEAD_FLAG_NONE;
					parsed_data_.version = VERSION_NONE;
					/*only remove header,not all frame,because lidar data may contain head*/
					for (int i = 0; i < 2; i++)
					{
						bin_buf_.erase(bin_buf_.begin());
					}
				}
			}
		}
		return true;
	}

	void OrdlidarDriver::mRxThreadProc(void *arg)
	{
		OrdlidarDriver *lidar_ptr = (OrdlidarDriver *)arg;
		unsigned char readbuf[TMPBUFF_SIZE] = {0};

		int epoll_num = 1;
		int retval = 0, actual_read = 0;
		struct epoll_event event;
		struct epoll_event wait_event;
		int ret = 1;

		int epoll_fd = epoll_create(epoll_num);
		if (epoll_fd != -1)
		{
			event.data.fd = lidar_ptr->serial_fd_;
			event.events = EPOLLIN;
			int ret = epoll_ctl(epoll_fd, EPOLL_CTL_ADD, lidar_ptr->serial_fd_, &event);
		}
		else
		{
			printf("epoll_create fail \n");
			goto exit;
		}

		while (!lidar_ptr->rx_thread_exit_flag_.load())
		{
			
			retval = epoll_wait(epoll_fd, &wait_event, epoll_num, 500);
			switch (retval)
			{
			case -1:
				printf(" epoll_wait error\n");
				break;
			case 0:
				lidar_ptr->valid_data_ = 0;
				// printf("time out\n");
				break;
			default:
			{
				if ((wait_event.data.fd == lidar_ptr->serial_fd_) && (EPOLLIN == (wait_event.events & EPOLLIN)))
				{
					actual_read = lidar_ptr->read_data(readbuf, TMPBUFF_SIZE);
					if (actual_read > 0)
					{
						lidar_ptr->valid_data_ = 1;
						lidar_ptr->uart_data_handle(readbuf, actual_read);
					}
				}
				break;
			}
			}
		}
	exit:
		if (epoll_fd >= 0)
		{
			close(epoll_fd);
		}
	}

	bool OrdlidarDriver::Connect()
	{
		if (isConnected())
		{
			Disconnect();
		}

		if (!serial_port_)
		{
			// serial_port_ = new serial::Serial(port_path, m_baudrate);
			serial_port_ = new serial::Serial();
		}

		serial_port_->uart_open(port_name_.c_str(), baudrate_, &serial_fd_);
		if (serial_fd_ > 0)
		{
			is_connected_ = true;
			rx_thread_exit_flag_ = false;
			rx_thread_ = new std::thread(mRxThreadProc, this);
		}
		else
		{
			is_connected_ = false;
		}

		return is_connected_;
	}

	bool OrdlidarDriver::isConnected() const
	{
		return is_connected_;
	}

	int OrdlidarDriver::read_data(unsigned char *data, int length)
	{
		return serial_port_->uart_read(serial_fd_, data, length);
	}

	int OrdlidarDriver::write_data(unsigned char *data, int length)
	{
		int ret = 0;

		ret = serial_port_->uart_write(serial_fd_, data, length);
		if (ret < 0)
			printf("lidar write error \n");

		return ret;
	}

	void OrdlidarDriver::Disconnect()
	{
		rx_thread_exit_flag_ = true;
		if (serial_fd_ > 0)
		{
			serial_port_->uart_close(serial_fd_);
			serial_fd_ = -1;
		}

		if ((rx_thread_ != nullptr) && rx_thread_->joinable())
		{
			rx_thread_->join();
			delete rx_thread_;
			rx_thread_ = NULL;
		}
		pthread_mutex_destroy(&one_scan_mtx_);
		pthread_cond_destroy(&one_scan_cond_);
		pthread_mutex_destroy(&full_scan_mtx_);
		pthread_cond_destroy(&full_scan_cond_);
		pthread_condattr_destroy(&cond_cattr_);
		is_connected_ = false;
	}

	uint16_t OrdlidarDriver::GetTimestamp() const
	{
		return parsed_data_.timestamp;
	}

	double OrdlidarDriver::GetRotationSpeed() const
	{
		return parsed_data_.speed;
	}
	
	bool OrdlidarDriver::SetRotationSpeed(uint16_t speed)
	{
		uart_comm_t request;
		uint16_t timeout = 0;
		double cur_speed = 0.0, min_thr = 0.0, max_thr = 0.0;
		bool return_value;
		int set_flag = 0;
		if(speed < 5)
			speed = 5;
		if(speed > 15)
			speed = 15;

		if(valid_data_)
		{
			set_flag = 1;
			return_value = Deactive();
			if(return_value != true)
			{
				return false;
			}
		}
        usleep(50*1000);
		request.head_flag = HEAD_FLAG;
		request.cmd = SET_ROTATION_SPEED;
		request.cmd_type = WRITE_PARAM;
		request.payload_len = 2;
		request.data[0] = speed & 0x00FF;
		request.data[1] = speed & 0xFF00;
		request.data[2] = xor_check((uint8_t*)&request, HEAD_LEN + request.payload_len);
		request.data[3] = 0x31;
		request.data[4] = 0xF2;
		int ret = serial_port_->uart_write(serial_fd_, (uint8_t*)&request, HEAD_LEN + request.payload_len + 3);
		if(ret == -1)
		{
			return false;
		}

		if(set_flag)
		{
			usleep(50*1000);
			return_value = Activate();
			if(return_value != true)
			{
				return false;
			}
		}
		else
		{
			return true;
		}

		min_thr = (double)speed - ((double)speed  * 0.1);
		max_thr = (double)speed + ((double)speed  * 0.1);
		return_value = false;
		while(timeout < SET_TIME_OUT)
		{
			sleep(1);
			cur_speed = GetRotationSpeed();
			if(valid_data_ == 0)
			{
				break;
			}
			if((cur_speed > min_thr) && (cur_speed < max_thr))
			{
				return_value = true;
				break;
			}
			timeout++;
		}

		return return_value;
	}

	bool OrdlidarDriver::GetFirmwareVersion(std::string &firmware_version) const
	{
		return false;
	}

	bool OrdlidarDriver::Activate(void)
	{
		uart_comm_t request;
		bool ret = false;
		uint16_t timeout = 0;

		if(valid_data_)
		{
			return true;
		}

		usleep(50*1000);						//防止连续调用，导致雷达无法处理
		request.head_flag = HEAD_FLAG;
		request.cmd = SET_RUN_MODE;
		request.cmd_type = WRITE_PARAM;
		request.payload_len = 1;
		request.data[0] = 0X81;
		request.data[1] = xor_check((uint8_t*)&request, HEAD_LEN + request.payload_len);
		request.data[2] = 0x31;
		request.data[3] = 0xF2;
		ret = serial_port_->uart_write(serial_fd_, (uint8_t*)&request, HEAD_LEN + request.payload_len + 3);
		if(ret == -1)
		{
			return false;
		}

		ret = false;
		while(timeout < SET_TIME_OUT)
		{
			sleep(1);
			if(valid_data_)
			{
				ret = true;
				break;
			}
			timeout++;
		}
		return ret;
	}
	bool OrdlidarDriver::Deactive(void)
	{
		uart_comm_t request;
		bool ret = false;
		uint16_t timeout = 0;
		if(valid_data_ == 0)
		{
			full_scan_data_.vailtidy_point_num = 0;
			one_scan_data_.vailtidy_point_num = 0;
			parsed_data_.timestamp = 0;
			parsed_data_.speed = 0;
			return true;
		}

		usleep(50*1000);						//防止连续调用，导致雷达无法处理
		request.head_flag = HEAD_FLAG;
		request.cmd = SET_RUN_MODE;
		request.cmd_type = WRITE_PARAM;
		request.payload_len = 1;
		request.data[0] = 0X80;
		request.data[1] = xor_check((uint8_t*)&request, HEAD_LEN + request.payload_len);
		request.data[2] = 0x31;
		request.data[3] = 0xF2;
		ret = serial_port_->uart_write(serial_fd_, (uint8_t*)&request, HEAD_LEN + request.payload_len + 3);
		if(ret == -1)
		{
			return false;
		}

		ret = false;
		while(timeout < SET_TIME_OUT)
		{
			sleep(1);
			if(valid_data_ == 0)
			{
				full_scan_data_.vailtidy_point_num = 0;
				one_scan_data_.vailtidy_point_num = 0;
				parsed_data_.timestamp = 0;
				parsed_data_.speed = 0;
				ret = true;
				break;
			}
			timeout++;
		}

		return ret;
	}

	bool OrdlidarDriver::GrabFullScanBlocking(full_scan_data_st &scan_data, int timeout_ms)
	{
#if 0
		int ret = 0;
		struct timespec timeout;
		struct timeval tv;
		gettimeofday(&tv, NULL);

		long long nsec = tv.tv_usec * 1000 + (timeout_ms % 1000) * 1000000;
		timeout.tv_nsec = nsec % 1000000000;
		timeout.tv_sec = tv.tv_sec + nsec / 1000000000 + timeout_ms / 1000;

		pthread_mutex_lock(&full_scan_mtx_);
		ret = pthread_cond_timedwait(&full_scan_cond_, &full_scan_mtx_, &timeout);
		pthread_mutex_unlock(&full_scan_mtx_);
		if (ret == 0)
		{
			if (IsFullScanReady())
			{
				scan_data = full_scan_data_;
				ResetFullScanReady();
				return true;
			}
		}
#else
		int ret = 0;
		struct timespec wait_time;
		clock_gettime(CLOCK_MONOTONIC, &wait_time);
		wait_time.tv_sec += timeout_ms / 1000;
		wait_time.tv_nsec += (timeout_ms % 1000) * 1000000ULL;

		if (wait_time.tv_nsec >= 1000000000)
		{
			++wait_time.tv_sec;
			wait_time.tv_nsec -= 1000000000;
		}

		pthread_mutex_lock(&full_scan_mtx_);
		ret = pthread_cond_timedwait(&full_scan_cond_, &full_scan_mtx_, &wait_time);
		pthread_mutex_unlock(&full_scan_mtx_);
		
		if (full_scan_data_.vailtidy_point_num == 0)
		{
			return false;
		}

		if (ret == 0)
		{
			if (IsFullScanReady())
			{
				scan_data = full_scan_data_;
				ResetFullScanReady();
				return true;
			}
		}
#endif
		return false;
	}

	bool OrdlidarDriver::GrabOneScanBlocking(one_scan_data_st &scan_data, int timeout_ms)
	{
#if 0
		int ret = 0;
		struct timespec timeout;
		struct timeval tv;
		gettimeofday(&tv, NULL);

		timeout.tv_sec = tv.tv_sec;
		timeout.tv_nsec = (tv.tv_usec * 1000 + timeout_ms) * 1000;

		pthread_mutex_lock(&one_scan_mtx_);
		ret = pthread_cond_timedwait(&one_scan_cond_, &one_scan_mtx_, &timeout);
		pthread_mutex_unlock(&one_scan_mtx_);
		if (ret == 0)
		{
			if (IsOneScanReady())
			{
				scan_data = one_scan_data_;
				ResetOneScanReady();
				return true;
			}
		}
#else
		int ret = 0;
		struct timespec wait_time;
		clock_gettime(CLOCK_MONOTONIC, &wait_time);
		wait_time.tv_sec += timeout_ms / 1000;
		wait_time.tv_nsec += (timeout_ms % 1000) * 1000000ULL;

		if (wait_time.tv_nsec >= 1000000000)
		{
			++wait_time.tv_sec;
			wait_time.tv_nsec -= 1000000000;
		}
		
		pthread_mutex_lock(&one_scan_mtx_);
		ret = pthread_cond_timedwait(&one_scan_cond_, &one_scan_mtx_, &wait_time);
		pthread_mutex_unlock(&one_scan_mtx_);

		if (one_scan_data_.vailtidy_point_num == 0)
		{
			return false;
		}

		if (ret == 0)
		{
			if (IsOneScanReady())
			{
				scan_data = one_scan_data_;
				ResetOneScanReady();
				return true;
			}
		}
#endif

		return false;
	}

	bool OrdlidarDriver::GrabFullScan(full_scan_data_st &scan_data)
	{
		if (full_scan_data_.vailtidy_point_num == 0)
		{
			return false;
		}

		if (!IsFullScanReady())
			return false;

		scan_data = full_scan_data_;
		ResetFullScanReady();
		return true;
	}

	bool OrdlidarDriver::GrabOneScan(one_scan_data_st &scan_data)
	{
		if (one_scan_data_.vailtidy_point_num == 0)
		{
			return false;
		}

		if (!IsOneScanReady())
			return false;

		scan_data = one_scan_data_;
		ResetOneScanReady();
		return true;
	}

}
